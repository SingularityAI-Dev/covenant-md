...
This is an npm workspaces monorepo implementing the COVENANT.md specification. ESM only (`"type": "module"`).

- **`packages/core/`** (`@covenant-md/core`), the library: validator, test runner, skill runner, lint, diff, graph. Public API exported from `packages/core/src/index.js`. Edit here.
- **`packages/cli/`** (`@covenant-md/cli`), the `covenant` CLI (bin) and the interactive generator. Depends on `@covenant-md/core`.
- **`examples/`**, four worked example skills (COVENANT.md + SKILL.md), exercised as fixtures.
- **`docs/COVENANT.md`**, the canonical specification. Treat as the source of truth when validator or test-runner behaviour is ambiguous.
...
```bash
npm ci                                                        # first-time setup (installs the workspace)
node packages/cli/src/cli.js validate examples/docx-generation/COVENANT.md
npm test                                                      # Jest across packages, under node --experimental-vm-modules
npm run test:fixtures                                         # runs the four example skills end-to-end through the CLI
node packages/cli/src/cli.js generate                         # interactive blueprint scaffold
```
...
The framework modules (validator, test runner, skill runner, lint, diff, graph in `packages/core/src/`; cli and generate in `packages/cli/src/`):
...
3. **`generate.js`**, Handlebars-driven interactive scaffolder using `inquirer`; emits a COVENANT.md + SKILL.md pair from `packages/cli/src/templates/`.
4. **`cli.js`**, `commander`-based entrypoint that wires the three together.

### The skillRunner contract

`test.js` does not execute skills itself; it delegates to a `skillRunner(skillName, operation, input)` function the caller supplies. `cli.js` builds that function by calling `createSkillRunner({ covenantPath })` from `@covenant-md/core` (`packages/core/src/skill-runner.js`), which inspects the skill's COVENANT.md and returns one of two strategies:
...
- Legacy: ` ```covenant-fixture ` fenced JSON blocks in the markdown body (see `packages/core/tests/fixtures/legacy-fixture-block.md` for an example).

## Conventions

- ESM only inside `packages/` (`import`/`export`, `.js` extensions in import paths).
- `covenant generate` scaffolds a new skill blueprint into the current directory; there is no checked-in empty target dir.
